...
class Instagram {
  constructor(
    ...
    { language, proxy, requestOptions, userAgent } = {}
  ) {
    this.credentials = {
      username,
      password,
    ...
    const userAgent = userAgent || useragentFromSeed(username)
  ...
  async _uploadPhoto({ photo }) {
    ...
    if (
      dimensionsImage.type !== 'jpg' ||
      dimensionsImage.width < 1080 ||
      dimensionsImage.height < 1080
    ) {
      const width = dimensionsImage.width < 1080 ? 1080 : dimensionsImage.width
      const height =
        dimensionsImage.height < 1080 ? 1080 : dimensionsImage.height
